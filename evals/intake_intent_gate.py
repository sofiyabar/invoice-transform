"""Intake Gate, Step 1 (formerly "Layer 0, Step 1") — Intent gate scoring:
how good is generator.intent_gate's is-invoice-intent decision compared to
ground truth?
...
already made, exactly like evals/field_accuracy.py only scores predictions
already generated (the generator call itself happens in evals/runner.py).

Two-function API, mirrors evals/field_accuracy.py (score_fields + aggregate_scores):
  ...
  aggregate_intent_scores()  -- rolls per-record results up into the actual
                                Intake Step 1 output: accuracy + FP rate +
                                FN rate.
...
re-prompt -- cheap friction), and Business Impact's formulas need them
apart, not blended into one number.
...
def score_intent(ground_truth: bool, prediction: bool) -> dict:
    ...
    error_type distinguishes the two failure modes that Business Impact
    prices differently -- see module docstring."""
...
def aggregate_intent_scores(per_record: list[dict]) -> dict:
    """Intake Step 1 output: accuracy, plus FP rate and FN rate kept apart.
